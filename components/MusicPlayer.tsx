...
export default function CloudPlayer({ songIds }: { songIds: string[] }) {
  ...
  return (
    <>
      <style>{`
        input[type=range]::-webkit-slider-thumb { -webkit-appearance: none; appearance: none; width: 12px; height: 12px; border-radius: 50%; background: #000000; cursor: pointer; transition: transform 0.1s; }
      ...
      <div className="md:col-span-5 rounded-3xl bg-white/40 dark:bg-slate-800/50 backdrop-blur-md border border-white/40 dark:border-white/10 shadow-xl p-6 flex flex-col justify-between transition-all duration-700 hover:scale-[1.02] relative group overflow-hidden min-h-[220px]">
        ...
        <div className={`absolute -top-20 -right-20 w-48 h-48 bg-black/20 blur-[50px] rounded-full transition-opacity duration-1000 ${isPlaying ? 'opacity-100' : 'opacity-30'}`}></div>
        ...
        <div className="flex items-center gap-5 relative z-10 mb-6 mt-2">
          ...
          <div className="flex-col overflow-hidden w-full">
            <div className="flex items-center justify-between mb-1">
              <span className="text-[10px] font-black text-black dark:text-white tracking-widest uppercase bg-white/50 dark:bg-slate-900/50 px-2 py-0.5 rounded-sm shadow-sm transition-colors duration-700">Cloud Music</span>
        ...
        <div className="relative z-10">
          <div className="flex items-center gap-3 text-xs text-slate-600 dark:text-slate-300 font-bold mb-3 transition-colors duration-700">
            ...
            <input
              ...
              style={{ background: `linear-gradient(to right, #000000 ${progress}%, rgba(255,255,255,0.2) ${progress}%)` }}
            />
            <span className="w-10">{formatTime(duration)}</span>
          </div>

          <div className="flex items-center justify-center gap-6">
            <button onClick={prevSong} className="text-slate-700 dark:text-slate-300 hover:text-black dark:hover:text-white transition-colors drop-shadow-sm"><svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24"><path d="M6 6h2v12H6zm3.5 6l8.5 6V6z"/></svg></button>
            <button onClick={togglePlay} className="w-12 h-12 bg-black text-white rounded-full flex items-center justify-center shadow-lg hover:bg-black hover:scale-110 transition-all border-2 border-white/50 dark:border-slate-600">
            ...
            <button onClick={nextSong} className="text-slate-700 dark:text-slate-300 hover:text-black dark:hover:text-white transition-colors drop-shadow-sm"><svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24"><path d="M6 18l8.5-6L6 6v12zM16 6v12h2V6h-2z"/></svg></button>
          </div>
        </div>
      </div>

      {/* 底部独立歌词长条：夜晚可以稍微更黑一点以凸显发光字体 */}
      <div className="md:col-span-12 order-last mt-4 rounded-3xl bg-slate-900/80 dark:bg-slate-950/90 backdrop-blur-xl border border-white/10 shadow-2xl p-5 flex items-center justify-between transition-all duration-700 hover:shadow-black/20 group min-h-[80px]">
        <div className="flex items-end justify-center gap-[4px] h-8 w-16">
          {isPlaying ? (
            <>
              <div className="w-1.5 bg-black rounded-t-sm safe-wave" style={{ animationDelay: '0ms' }}></div>
              <div className="w-1.5 bg-black rounded-t-sm safe-wave" style={{ animationDelay: '200ms' }}></div>
              <div className="w-1.5 bg-black rounded-t-sm safe-wave" style={{ animationDelay: '400ms' }}></div>
              <div className="w-1.5 bg-black rounded-t-sm safe-wave" style={{ animationDelay: '100ms' }}></div>
              <div className="w-1.5 bg-black rounded-t-sm safe-wave" style={{ animationDelay: '300ms' }}></div>
            </>
          ) : (
            <>
        ...
        <div className="flex-1 px-8 flex justify-center items-center overflow-hidden">
          <p className="text-white text-lg font-bold tracking-widest truncate drop-shadow-[0_0_8px_rgba(0,0,0,0.8)]">
            {displayedLyric}
            <span className="inline-block w-[3px] h-5 bg-black align-middle ml-1 shadow-[0_0_8px_rgba(0,0,0,0.8)] animate-cursor"></span>
          </p>
        </div>

        <div className="w-16 flex justify-end">
          <svg className={`w-6 h-6 text-black/50 ${isPlaying ? 'animate-bounce' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
