    ...
    >
      ...
      <div className="flex flex-col md:flex-row items-end md:items-center justify-between mt-8 gap-6 relative z-10">
        ...
        <div className="flex gap-6 w-full md:w-auto justify-around md:justify-start">
          ...
          <StatItem count={momentCount} label="说说" color="text-dark-300 dark:text-dark-400" />
          <div className="w-px h-10 bg-slate-300/50 dark:bg-slate-700 hidden md:block"></div>
...
function SocialBtn({ type, url, onClick }: { type: string, url?: string, onClick?: () => void }) {
  const content = (
    <div
      onClick={onClick}
      className="w-10 h-10 rounded-xl bg-white/50 dark:bg-slate-700/50 flex items-center justify-center text-slate-700 dark:text-slate-300 hover:bg-black hover:text-white dark:hover:bg-black dark:hover:text-white transition-all duration-300 border border-white/40 dark:border-white/10 shadow-sm"
      title={type}
    >
